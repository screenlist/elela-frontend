<script>
  ...
  import { getAccount, createConfig, http, writeContract, watchAccount, getBalance } from "@wagmi/core"
  ...
  $inspect(account).with(console.log)
...
<div class="flex flex-col justify-items-start items-center p-4">
  <div class="card card-border bg-base-200 max-w-lg w-full min-w-xs">
    <section class="card-body">
      <h1 class="card-title">Buy {data.crypto.quantity} drops @ {data.crypto.price} {data.crypto.currency}</h1>
      <div role="alert" class="alert alert-soft alert-warning">
        <Info />
        <span>Do not reload this page.</span>
      </div>
      <div class="card-actions flex-row">
        ...
        {:else}
          <button onclick={connect} class="btn btn-neutral">Connect</button>
        {/if}
      </div>
    </section>
  </div>
